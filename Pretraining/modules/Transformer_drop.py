...
class MultiHeadAttention_drop(nn.Module):
    ...
    def forward(self, q, k, v, mask=None, dropout=None):
        ...
        output = self.scaled_dot_product_attention(q, k, v, self.d_k, mask, dropout)
...
class TransformerLayer_drop(nn.Module):
    def __init__(self, d_model, d_ff, n_heads, dropout, kq_same=False):
        ...
        self.masked_attn_head = MultiHeadAttention_drop(d_model, n_heads, kq_same=kq_same)
    ...
    def forward(self, seq, mask=None):
        context = self.masked_attn_head(seq, seq, seq, mask, self.dropout3)
