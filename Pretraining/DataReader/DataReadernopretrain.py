...
###read a subdataset in combine formate, no need to map
class DataReadernopretrain(object):
    ...
    def load_nopretrain_data(self):
        for key in ['train', 'dev', 'test']:
            self.data_df[key] = pd.read_csv(os.path.join(self.prefix, self.dataset, key + '.csv'), sep=self.sep)
            self.data_df[key] = utils.eval_list_columns(self.data_df[key])
        self.category_dict = pickle.load(open(os.path.join(self.prefix, self.dataset, 'category.pkl'), 'rb'))
        self.category_size = max(set(self.category_dict.values())) + 1
        ###create nearest POI
        self.coordinate_dict = pickle.load(
            open(os.path.join(self.prefix, self.dataset, 'coordinate.pkl'),
                 'rb'))
        POI_nearest_path = os.path.join(self.prefix, self.dataset, 'POI_nearest.pkl')
        if not os.path.exists(POI_nearest_path):
            (self.POI_nearest, self.POI_nearest_distance) = self.create_nearest(self.coordinate_dict, POI_nearest_path)
        else:
            (self.POI_nearest, self.POI_nearest_distance) = pickle.load(open(POI_nearest_path, 'rb'))
    ...
    def create_nearest(self, coordinate_dict, POI_nearest_path):
        ...
        return (POI_nearest, POI_nearest_distance)
    ...
    def calculate_distance(self, cluster_center_matrix, coor_matrix, Batch_size):
        ...
        new_clusters = np.zeros((coor_num, 100))  ###N个
        new_clusters_distance = np.zeros((coor_num, 100))
        for it in range(batch_num):
            ...
            cluster_center = np.argsort(distance_matrix, axis=-1)[:, 1:101]
